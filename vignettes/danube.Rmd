...
As we have a matrix with the flow connection information we are going to illustrate how we can use it for estilation and later for post-estimation plots. 
...
We create an object of class `FlowConnectionMatrix`. We do this by passing the graph and the matrix to the constructor. The constructor exxecutes some checks between the matrix and the graph. However it does not check whether the matrix is correct with respect to the graph. 
...
Such a collection of subsets may have bies reduction effect on the estimates.
...
Using the library `corrplot` we can create a palette of colours: from red (strong extremal dependence) to greeen (weak extremal dependence).
...
The upper tringular part presents the method of moment estimates and the lower triangular part the extremal coefficients estimates. The white plusses indicate if two nodes are flow connected. 
