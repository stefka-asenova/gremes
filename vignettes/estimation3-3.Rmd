...
The pairwise extremal coefficients estimator, named at the beginning EKS to the authors of the paper where it appeared @eks16, is based on the bivariate stable tail dependence function (stdf). It is decribed in Section 4.3 in  @asenova2021. More on the stdf can be found in @haan.
...
The second version of the ECE which is imlemented with object of class `EKS_part` uses the subsets $W_u$ for every $u\in U$ where $U$ is the set of noted with observable variables. It is similar to the MLE Version 1 explained in Vignette "Estimation - Note 2".
